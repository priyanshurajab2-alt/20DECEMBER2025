...
def api_question_status(test_id):
    ...
    # 🔥 YOUR EXACT DB HANDLING (from api_single_question)
    db_file = request.args.get('db_file') if request.method == 'GET' else (request.form.get('db_file') if request.method == 'POST' else None)
    if db_file:
        db_path = f"/var/data/{db_file}"
        if os.path.exists(db_path):
            conn = sqlite3.connect(db_path)
