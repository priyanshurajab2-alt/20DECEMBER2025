...
def api_question_status(test_id):
    ...
    # 🔥 CONSTANT - Your admin_users.db path
    ...
    if db_file:
        ...
        if os.path.exists(db_path):
            ...
            # CREATE TABLE (your existing code - PERFECT)
            conn.execute('''
                CREATE TABLE IF NOT EXISTS question_status (
                    ...
                    FOREIGN KEY (test_id, question_id) REFERENCES test_questions (test_id, id),
                    UNIQUE(user_id, test_id, question_id)
                )
            ''')
            print("✅ question_status table ready")
        else:
            return jsonify({"error": f"Database not found: {db_file}"}), 404
    else:
        conn = get_session_db(test_id)
        if not conn:
            return jsonify({"error": "No database specified"}), 400
        conn.row_factory = sqlite3.Row
        # Same CREATE TABLE for session DB...

    try:
        # 🔥 FIXED USER ID - Link admin_users.db (REPLACE your session line)
        user_email = request.args.get('user_email') or session.get('user_email')
        if user_email and os.path.exists(USER_DB_FILE):
            admin_conn = sqlite3.connect(USER_DB_FILE)
            admin_cursor = admin_conn.execute(
                "SELECT id FROM users WHERE email = ? LIMIT 1", 
                (user_email,)
            )
            user_row = admin_cursor.fetchone()
            user_id = user_row[0] if user_row else 1
            admin_conn.close()
            print(f"✅ email={user_email} → user_id={user_id}")
        else:
            user_id = request.args.get('user_id') or session.get('user_id', 1)
            print(f"🔍 Fallback user_id={user_id}")
        ...
        if request.method == 'POST':
            ...
            # 🔥 ALL ACTIONS use CORRECT user_id now
            if action == 'answer':
        ...
        # 🔥 GET - Your existing queries (PERFECT - now with correct user_id)
        test = conn.execute('SELECT * FROM test_info WHERE id = ?', (test_id,)).fetchone()
        questions = conn.execute(
            'SELECT id, subject, topic, question FROM test_questions WHERE test_id = ? ORDER BY id',
            (test_id,)
        ).fetchall()
        
        marked_questions = [row[0] for row in conn.execute("""
            SELECT question_id FROM question_status 
            WHERE user_id = ? AND test_id = ? AND marked_for_review = 1
        """, (user_id, test_id)).fetchall()]
        
        skipped_questions = [row[0] for row in conn.execute("""
            SELECT question_id FROM question_status 
            WHERE user_id = ? AND test_id = ? AND visited = 1 
            AND selected_option IS NULL AND marked_for_review = 0
        """, (user_id, test_id)).fetchall()]
        
        answers_list = conn.execute("""
            SELECT question_id, selected_option FROM question_status 
            WHERE user_id = ? AND test_id = ? AND selected_option IS NOT NULL
        """, (user_id, test_id)).fetchall()
        answers = {str(row[0]): row[1] for row in answers_list}
...
def api_review_attempted(test_id):
    ...
    try:
        ...
        correct_questions = [dict(q) for q in all_questions if q['user_answer'] is not None and q['is_correct'] == 1 and q['is_skipped'] == 0]
        incorrect_questions = [dict(q) for q in all_questions if q['user_answer'] is not None and q['is_correct'] == 0 and q['is_skipped'] == 0]
        skipped_questions = [dict(q) for q in all_questions if q['user_answer'] is None and q['is_correct'] == 0 and q['is_skipped'] == 1]
        unanswered_questions = [dict(q) for q in all_questions if q['user_answer'] is None and q['is_correct'] == 0 and q['is_skipped'] == 0]

        
        print(f"✅ Correct={len(correct_questions)}, Wrong={len(incorrect_questions)}")
