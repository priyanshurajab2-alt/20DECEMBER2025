...
def api_test_review(test_id):
    """FLUTTER API - 6x6 Review Grid Data"""
    import sqlite3
    ...
    # 🔥 SAME AS api_single_question: Direct DB path
    full_db_path = f"/var/data/{db_file}"
    print(f"🔥 DEBUG: review connecting to {full_db_path}")
    ...
    try:
        ...
        # 🔥 Session data (Mark/Answer/Skip tracking)
        answer_key = f'test_{test_id}_answers'
        mark_key = f'test_{test_id}_marked'
        skip_key = f'test_{test_id}_skipped'
    ...
    # 🔥 PERFECT Flutter format
    return jsonify({
        'success': True,
        'test': dict(test),
        'questions': [dict(q) for q in questions],
        'answers': answers,
        'marked_questions': list(marked),      # Flutter expects this name
        'skipped_questions': list(skipped),    # Flutter expects this name
    })
...
@test_bp.route('/api/tests/<int:test_id>/question-status', methods=['GET', 'POST'])
def api_question_status(test_id):
    data = request.get_json() if request.method == 'POST' else None
    
    # 🔥 SAME PATTERN AS YOUR SINGLE QUESTION ROUTE
    db_file = request.args.get('db_file') if request.method == 'GET' else data.get('db_file')
    if db_file:
        db_path = f"/var/data/{db_file}"
        if os.path.exists(db_path):
            conn = sqlite3.connect(db_path)
    ...
    user_id = session.get('user_id', 1)  # Your auth
    ...
    if request.method == 'POST':
        ...
        if action == 'answer':
            conn.execute("""
                ...
                (user_id, test_id, question_id, selected_option, visited, updated_at)
                VALUES (?, ?, ?, ?, 1, CURRENT_TIMESTAMP)
            """, (user_id, test_id, question_id, data['answer']))
        elif action == 'mark':
            conn.execute("""
                INSERT OR REPLACE INTO question_status 
                (user_id, test_id, question_id, visited, marked_for_review, updated_at)
                VALUES (?, ?, ?, 1, 1, CURRENT_TIMESTAMP)
            """, (user_id, test_id, question_id))
        elif action == 'skip':
            conn.execute("""
                INSERT OR IGNORE INTO question_status 
                (user_id, test_id, question_id, visited, updated_at)
                VALUES (?, ?, ?, 1, CURRENT_TIMESTAMP)
            """, (user_id, test_id, question_id))
    ...
    # GET - your Flutter format
    try:
        test = conn.execute("SELECT * FROM test_info WHERE id = ?", (test_id,)).fetchone()
        questions = conn.execute("SELECT * FROM test_questions WHERE test_id = ? ORDER BY id", (test_id,)).fetchall()
        
        marked_questions = conn.execute("""
            SELECT qs.question_id FROM question_status qs 
            WHERE qs.user_id = ? AND qs.test_id = ? AND qs.marked_for_review = 1
        """, (user_id, test_id)).fetchall()
            
        skipped_questions = conn.execute("""
            SELECT qs.question_id FROM question_status qs 
            WHERE qs.user_id = ? AND qs.test_id = ? AND qs.visited = 1 
            AND qs.selected_option IS NULL AND qs.marked_for_review = 0
        """, (user_id, test_id)).fetchall()
        
        answers = conn.execute("""
            SELECT qs.question_id, qs.selected_option FROM question_status qs 
            WHERE qs.user_id = ? AND qs.test_id = ?
        """, (user_id, test_id)).fetchall()
        answers_dict = {str(row[0]): row[1] for row in answers if row[1]}
        ...
        return jsonify({
            ...
            'marked_questions': [row[0] for row in marked_questions],
            'skipped_questions': [row[0] for row in skipped_questions],
        })
    finally:
        conn.close()
