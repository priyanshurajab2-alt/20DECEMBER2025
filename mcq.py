    ...
    try:
        ...
        if topic_id:
            ...
            question_data = source_conn.execute(
                "SELECT question, option_a, option_b, option_c, option_d, explanation, correct_answer FROM mcq_questions WHERE id=?", (question_id,)
            ).fetchone()
            ...
            explanation, user_response, is_correct, correct_option)  # 🔥 ADDED correct_option
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)  
                """, (topic_id, user_id, question_id, 
                    ...
                    user_response,outcome,correct_answer['correct_answer']))
